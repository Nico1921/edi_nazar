<script setup>
var props = defineProps(['name','titre','max','value']);
...
<template>
  <div>
    <label v-if="titre != ''" for="custom-input-number" class="w-full text-gray-700 text-sm font-semibold">{{titre}}
      ...
      <div class="custom-number-input flex flex-row min-h-full h-full w-full rounded-lg relative bg-transparent">
        <div >
         ...
         class="border-none outline-none focus:outline-none text-center w-full bg-gray-300 font-semibold text-md hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-700  outline-none" 
         :name="name" 
         :max="max"
         min="1"
