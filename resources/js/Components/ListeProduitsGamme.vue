...
var props = defineProps(['products','ifEdit','url_add','url_delete','type_vente','slidePerViewMount','slidePerView','classGridImage','classGridText','hBlock']);
...
var addCommande = (e,isPanier) => {
   ...
   var formProduit = useForm();
   if(props.type_vente == 2){
      formProduit = useForm({
   ...
   };console.log(props.url_add);
   formProduit.post(props.url_add, {
      preserveScroll: true,
      preserveState:true,
      onSuccess: (e) => {
...
var deleteCommande = (id_panier_edi_list,key) =>{
   Swal.fire({
      ...
      if (result.isConfirmed) {
         ...
         formProduit.post(props.url_delete, {
            preserveScroll: true,
            onSuccess: (e) => {
               if(e.props.session.status){
                  Toast.fire({
...
onMounted(() => {
   ...
   swiperEl.setBreakpoint(props.slidePerViewMount);
   swiperEl.update();
});
</script>
               ...
               <div class=" bg-gray-200 grid grid-cols-12 px-4 py-4" :class="hBlock" >
                  ...
                  <div :class="props.classGridText" class="px-2 grid grid-cols-1">
                     ...
                     <div class="grid grid-cols-12">
                        <div class="2xl:pr-1 sm:col-span-1 col-span-2">
                        ...
                        <div class="2xl:pr-1 sm:col-span-11 col-span-10">
                           <div v-if="design.stats_produit.stock_restant > 10">
                     ...
                     <div v-if="typeVente == props.type_vente" class="mt-2">
                        <form @submit.prevent="addCommande($event,(design.panier.quantiter > 0 ? true : false))" class="grid grid-cols-8"
                           ...
                           <div class="sm:col-span-3 col-span-8 mr-2">
                              <InputNumber class="sm:h-10 h-8 sm:w-full w-32" name="qte" :max="design.stats_produit.stock_restant" :value="(design.panier.quantiter > 0 ? design.panier.quantiter : 1)" />
                           </div>
                           <div class="sm:col-span-5 col-span-8">
                              <div class="h-10 flex items-start">
                                 <button type="submit" class="sm:text-sm text-[0.700rem] font-bold sm:h-full h-3/4 sm:mt-0 mt-2 sm:px-3 px-2 bg-primary-50 rounded hover:bg-primary-100 transition duration-300">{{ design.panier.quantiter > 0 ? 'Modifier' : 'Ajouter à' }} la commande</button>
                              </div>
                           </div>
                        </form>
                        <div v-else-if="design.isInPanier" class="col-span-2 grid grid-cols-2">
                           ...
                           <div class="grid grid-cols-8 col-span-2" v-if="props.ifEdit">
                              ...
                              <div class="xl:col-span-3 sm:col-span-4 col-span-8 sm:mt-0 mt-1 grid grid-cols-2">
                                 <div class="col-span-1 sm:mx-1 mr-1 xl:mb-0 mb-2 ">
                                    <button type="button" @click="editCommande(key)"
                                       class="px-2 h-8 bg-primary-50 rounded hover:bg-primary-100 transition duration-300 w-full">
                                 ...
                                 <div class="col-span-1">
                                    ...
                                    class="px-2 h-8 bg-red-500 text-white rounded hover:bg-red-300 transition duration-300 w-full">
